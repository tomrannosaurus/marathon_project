...
fixed_effects %>% 
  ...
  kbl(
  digits = 3,
  caption = "Fixed Effects of Mixed-Effects Model for Aim 1",
  booktabs = TRUE)
...
ggplot(new_data, aes(x = Age, y = Predicted, color = Sex)) +
  ...
  labs(
    title = "Predicted Running Times by Age and Sex with Peak Performance",
    x = "Age",
...
```{r performance v weather, fig.cap="Performance vs. Weather by Sex", echo = F, eval = T, fig.height=2.6, fig.width=7.5}
#| label: fig-env
...
ggplot(env_vars_long, aes(x = Value, y = norm_diff, color = Sex)) +
  ...
  labs(y = "Normalized Difference in % CR",
       color = "Sex") +
  theme_minimal() +
  scale_color_manual(values = c("Female" = "lightpink2", "Male" = "lightblue2")) +
  guides(color = guide_legend(override.aes = list(linewidth = 1.5))) +
...
fixed_effects %>% 
  ...
  kbl(
  digits = 3,
  caption = "Significant Fixed Effects of Mixed-Effects Model for Aim 2",
  booktabs = TRUE
)
...
fixed_effects %>% 
  ...
  kbl(
  digits = 3,
  caption = "Significant Fixed Effects of Model for Aim 2",
  booktabs = TRUE
)
